...
export default {
	...
	computed: {
		...
		isFavourite() {
           return this.$store.state.favourites.find(item => item.adId == this.adId) ? true : false;
        },
		favouriteDate() {
             return this.$store.state.favourites.find(item => item.adId == this.adId).date;
        }
	}
};
</script>
