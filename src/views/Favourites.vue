...
export default {
  ...
  methods: {
    orderList(orderType) {
      if (orderType == "address") {
        this.sortedList = this.favourites.sort((a, b) =>
          a.address > b.address ? 1 : -1
        );
      }
      if (orderType == "date") {
        this.sortedList = this.favourites.sort((a, b) =>
          a.favouriteDate - b.favouriteDate ? 1 : -1
        );
      }
    },
  },
  mounted() {
    ...
    var favourites = [];
    favouritesIds.forEach((element) =>
      favourites.push(
        this.$store.state.ads.find((item) => item.adId == element)
      )
    );
    for (var i = 0; i < favouritesIds.length; i++) {
      favourites.find((item) => item.adId == favouritesIds[i]).favouriteDate =
        this.$store.state.favourites[i].date;
    }
